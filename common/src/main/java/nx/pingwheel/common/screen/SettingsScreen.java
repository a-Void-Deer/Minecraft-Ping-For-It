...
public class SettingsScreen extends OptionsSubScreen {
	...
	public void render(GuiGraphics ctx, int mouseX, int mouseY, float delta) {
		...
		if (this.channelTextField.isHoveredOrFocused() && !this.channelTextField.isFocused()) {
			var list = this.font.split(LanguageUtils.settings("channel.tooltip").get(), 140);
			var clientTooltipComponentList = list.stream().map(a -> ClientTooltipComponent.create(a)).collect(Collectors.toUnmodifiableList());
